          ...
          <div key={b.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white">
            ...
            <div className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex gap-1">
                <button onClick={() => { setEditing(b); setShowForm(true); }} className="rounded p-1.5 text-gray-500 hover:bg-gray-100"><Pencil className="h-4 w-4" /></button>
                <button onClick={() => handleDelete(b.id)} className="rounded p-1.5 text-red-500 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          </div>
        ))}
