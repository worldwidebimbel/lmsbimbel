...
async function getAssignmentWithBranch(id: string) {
  ...
  if (!isSuperAdmin && branchId && assignment.class.branchId !== branchId) {
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { allowed } = await getAssignmentWithBranch(id);
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const { assignment, allowed } = await getAssignmentWithBranch(id);
  if (!assignment) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!allowed || assignment.teacherId !== session.user.id) {
