    ...
    <div className={`rounded-xl border p-4 space-y-3 ${past ? "border-gray-100 bg-gray-50/50" : "border-gray-200 bg-white"}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button onClick={() => onEdit(session)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600">
            <Pencil className="h-4 w-4" />
          </button>
          <button onClick={() => onDelete(session.id)} disabled={deleting === session.id} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-500">
