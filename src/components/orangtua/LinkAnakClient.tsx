...
export default function LinkAnakClient({ initialChildren }: { initialChildren: Student[] }) {
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await fetch("/api/orangtua/link-anak", {
        method: "POST",
      ...
      setEmail("");
      setSuccess(`${data.name} berhasil ditambahkan!`);
    });
  }

  async function handleRemove(childId: string, name: string) {
    if (!confirm(`Lepas hubungan dengan ${name}?`)) return;
    const res = await fetch("/api/orangtua/link-anak", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        ...
        {success && (
          <div className="mb-2 rounded-lg bg-green-50 border border-green-200 px-4 py-2">
            <p className="text-sm text-green-700">{success}</p>
          </div>
        )}
        <form onSubmit={handleAdd} className="flex items-end gap-3">
          <div className="flex-1">
            <label className="mb-1.5 block text-sm font-medium text-gray-700">Email Anak</label>
            <input
              type="email"
              value={email}
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3">
          <h3 className="font-semibold text-gray-900">Anak yang Terhubung</h3>
