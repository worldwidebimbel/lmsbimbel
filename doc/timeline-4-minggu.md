...
- [x] Alur Akademik: jurnal mengajar → raport generate → publish → orang tua lihat → cetak PDF — audit kode menyeluruh: fix upsert composite unique (findFirst + update/create), hapus duplikasi field academicYearId, guard kelas tanpa tutor (PDF & halaman siswa/ortu), isolasi cabang di detail/update/delete/PDF/export (admin cabang scope cabangnya, guru hanya kelasnya), UI RaportAdminClient lengkap (generate/publish/unpublish/edit catatan/filter); panduan `doc/guide-alur-akademik.md`
- [x] Alur Payroll: absensi tutor → generate payroll → approve → slip PDF — audit kode menyeluruh: generate idempoten (update DRAFT, skip APPROVED/PAID), proteksi payroll PAID dari modifikasi/hapus, validasi transisi status (DRAFT→APPROVED→PAID), isolasi cabang, notifikasi status ke tutor, slip PDF + tombol download + export Excel; panduan `doc/guide-alur-payroll.md`
- [x] Alur Keuangan: invoice → bayar online → webhook → laporan pendapatan per program — audit kode menyeluruh: fix Midtrans callback tanpa signature verification (SHA512 server_key), fix parsing order_id CUID ber-hyphen (slice(1,-1)), tambah handler PPDB di Midtrans callback, buat webhook Xendit baru (sebelumnya tidak ada → pembayaran Xendit tidak pernah terkonfirmasi), BranchTransaction untuk semua jalur pembayaran (Duitku/Midtrans/Xendit webhook + admin approve/confirm + PPDB), guard PENDING di semua checkout (cegah double-checkout), route pay-online orang tua (sebelumnya tombol UI mati), fix filter Payment branchId (harus via relasi invoice) di export & laporan; panduan `doc/guide-alur-keuangan.md`
